...
| website  | string |new website to change|
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| baseAsset  | string |base asset's token symbol|
| quoteAsset  | string |quote asset's token symbol|
| initPriceStr  | string |the init price of the tokenpair that will be listed|
...
##### 7.2.4 Deposit some tokens to a specific product
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| product  | string |token pair's name|
| amountStr  | string |amount to deposit, e.g. "1024.1024okt"|
...
##### 7.2.5 Withdraw some tokens from a specific product
...
##### 7.2.6 Transfer the ownership of a product
...
| seqNum  | uint64 |sequence number of sender's account on chain|
| product  | string |token pair's name|
| toAddrStr  | string |the user's account in bech32 to be transferred|
